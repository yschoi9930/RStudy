...
# x 축범위 3~6, y 축 범위 10~30
# +뒤에서 줄바꾸기 실행 : +연산자 행 앞으로 오면 에러
ggplot(mpg,aes(displ,hwy)) + 
  geom_point() +
  xlim(3,6) +
  ylim(10,30)

# 고속도로연비와 도시연비간의 관계 알아보기(산점도)
...
library(dplyr)
#  구동방식별 고속도로 연비의 평균이 얼마나 차이가 나는지 확인
...
# geom_bar() : 빈도 막대 그래프
# 막대 그래프의 x의 위치에 나타나느 각 그룹의 빈도수를 나타내는 막대그래프
...
#------------------------
# 막대 그래프 2 - 빈도 막대 그래프
# 값의 개수(빈도)로 막대 길의 표현한 그래프

# 1. 범주 변수인 경우
#  x 축 : 범주 변수, y 축 : 빈도
table(mpg$drv) #drv의 빈도 확인
# 4   f   r 
# 103 106  25 
ggplot(mpg,aes(x=drv)) + geom_bar()
# ggplot(mpg,aes(x=drv)) + geom_col() # y데이터 필요
str(mpg$drv)
...
# Q1. 어떤 회사에서 생산한 "suv" 차종의 
# 도시 연비가 높은지 알아보려고 합니다. 
# "suv" 차종을 대상으로 
# 평균 cty(도시 연비)가 가장 높은 회사 다섯 곳을 
# 막대 그래프로 표현해 보세요. 
...
ggplot(sleep, aes(ID, extra,fill=group)) +
  geom_bar(stat='identity') + # position 생략 : 누적 가로 막대그래프
  coord_flip()
...
#   summarize(Sum.circumference = sum(circumference)) %>%
#   ggplot(aes(Tree, Sum.circumference)) + 
#   geom_bar(stat='identity',fill=Tree) #Tree 변수값을 찾을 수 없음

# fill 위치
# fill = 색상 : aes() 안에 있어도 되고, 
# geom_bar() 안에 있어도 됨 
# 단, 색상값이 아닌 변수 값을 그래프의 색상을 결정하고자 한다면
# aes() 함수 안에서 사용되어야 함
...
# 전체 데이터에 대해 나무의 나이별 성장을 나타낸 그래프
Orange %>% 
...
# 색상 팔레트 : palette를 지정하면 색상이 순서대로 지정
# scale_fill_brewer : 파레트 지정하는 함수
...
#------------- 막대 그래프에 레이블 추가
# 빈도 막대 그래프
# geom_text(stat="count") 변수 ..count..를 사용해서 레이블출력
...
ggplot(mtcars, aes(x=factor(cyl),y=..count..,fill=as.factor(am))) +
  ...
  geom_text(stat="count",
            aes(label=..count..),
            position=position_dodge(width=0.5), #geom_bar()position 인수와 같은 함수를 사용
            vjust=-0.5)


ggsave("a.jpg", dpi=300)
